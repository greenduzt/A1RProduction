...
{
    public class PrintAllMachineWorkOrdersPDF
    {
        Document document;
        ...
        private List<MachineWorkOrder> machineWorkOrderList;
        private List<MachineWorkDescription> machineWorkDescriptionList;
        private List<MachineRepairDescription> machineRepairDescription;
        private List<MachineParts> machineParts;
        ...
        //private List<MachineWorkOrder> vehicleWorkOrderList;
        //private ObservableCollection<MachineWorkDescription> vehicleWorkOrderDescription;

        public PrintAllMachineWorkOrdersPDF(List<MachineWorkOrder> mwol, List<MachineWorkDescription> mwdl, List<MachineRepairDescription> mrd, List<MachineParts> mp,string fn)
        {
            machineWorkOrderList = mwol;
            machineWorkDescriptionList = mwdl;
            machineRepairDescription = mrd;
            machineParts = mp;
            fullName = fn;
        ...
        public Exception createWorkOrderPDF()
        {
            ...
            string filename = "ALLMWO" + currentDate.ToString("_ddMMyyyy_HHmmss") + ".pdf";
            try
            {
        ...
        void CreatePage()
        {
            foreach (var item in machineWorkOrderList)
            {
                ...
                else if (!string.IsNullOrWhiteSpace(item.OrderType) && item.OrderType.Equals("External"))
                {
                    header = "External";
                }
                ...
                paragraph = this.addressFrame.AddParagraph("Work Order Type - " + item.WorkOrderType);
                paragraph.Format.Font.Name = "Calibri";
                ...
                paragraph = this.addressFrame.AddParagraph("Maintenance Frequency - " + item.MachineMaintenanceFrequency.Frequency);
                paragraph.Format.Font.Name = "Calibri";
                paragraph.Format.Font.Size = 12;
                //paragraph.Format.SpaceBefore = "2cm";
                paragraph.Format.Font.Bold = true;
                ...
                paragraph = this.addressFrame.AddParagraph("Work Order No - " + item.WorkOrderNo);
                paragraph.Format.Font.Name = "Calibri";
                paragraph.Format.Font.Size = 12;
                paragraph.Format.Font.Bold = true;
                paragraph.Format.Alignment = ParagraphAlignment.Right;

                paragraph = this.addressFrame.AddParagraph("Assigned Date - " + Convert.ToDateTime(item.NextServiceDate).ToString("dd/MM/yyyy"));
                paragraph.Format.Font.Name = "Calibri";
                paragraph.Format.Font.Size = 12;
                paragraph.Format.Font.Bold = true;
                paragraph.Format.Alignment = ParagraphAlignment.Right;
                ...
                row4.Cells[0].AddParagraph(item.Machine.MachineID.ToString());
                ...
                row4.Cells[1].AddParagraph(item.Machine.MachineName);
                ...
                row4.Cells[2].AddParagraph(item.Machine.MachineType);
                ...
                row4.Cells[3].AddParagraph(fullName);
                ...
                if (item.WorkOrderType == VehicleWorkOrderTypesEnum.Maintenance.ToString())
                {
                ...
                foreach (var itemMWDL in machineWorkDescriptionList)
                {
                    if (itemMWDL.MachineWorkOrderNo == item.WorkOrderNo)
                    {
                        ...
                        if (item.WorkOrderType == VehicleWorkOrderTypesEnum.Maintenance.ToString())
                        {
                            row2.Cells[0].AddParagraph(itemMWDL.MachineMaintenanceInfo.MachineCode);
                            ...
                            row2.Cells[1].AddParagraph(itemMWDL.MachineMaintenanceInfo.MachineDescription);
                            ...
                            if (item.WorkOrderType == VehicleWorkOrderTypesEnum.Repair.ToString())
                            {
                            ...
                            {
                                //Console.WriteLine(item.WorkOrderNo + " " + itemMWDL.MachineMaintenanceInfo.MachineCode + " " + itemMWDL.MachineMaintenanceInfo.MachineDescription);
                            ...
                            if (item.WorkOrderType == VehicleWorkOrderTypesEnum.Maintenance.ToString())
                            {
                                foreach (var itemMRD in machineRepairDescription)
                                {
                                    if (itemMWDL.ID == itemMRD.MachineWorkDescriptionID)
                                    {
                                        ...
                                        vrdRow.Cells[1].AddParagraph(itemMRD.StrSequenceNumber + "  |  " + itemMRD.RepairDescription);
                                        ...
                                        foreach (var itemMP in machineParts)
                                        {
                                            if (itemMRD.ID == itemMP.MachineRepairID)
                                            {
                                                ...
                                                vpRow.Cells[2].AddParagraph(itemMP.StrSequenceNumber + "  |  " + itemMP.PartCode);
                        ...
                        else if (item.WorkOrderType == VehicleWorkOrderTypesEnum.Repair.ToString())
                        {

                                    row2.Cells[0].AddParagraph(itemMWDL.MachineMaintenanceInfo == null ? "Repair order for " + item.Machine.MachineName : itemMWDL.MachineMaintenanceInfo.MachineDescription);
                                    row2.Cells[0].Format.Font.Size = 9;
                                    ...
                                    foreach (var itemMRD in machineRepairDescription)
                                    {
                                        if(itemMWDL.MachineWorkOrderNo == itemMRD.MachineWorkOrderID)
                                        {
                                            Row vrdRow = table.AddRow();
                                            vrdRow.HeadingFormat = true;
                                            vrdRow.Format.Alignment = ParagraphAlignment.Center;
                ...
                MachineSpecialRequiremantsList = DBAccess.GetMachineSpecialReqByMachId(item.Machine.MachineID);
                if (MachineSpecialRequiremantsList.Count > 0)
                {
                    ...
                    int x = 1;
                    foreach (var itemSR in MachineSpecialRequiremantsList)
                    {
                        ...
                        specRow2.Cells[1].AddParagraph(itemSR.Description);
