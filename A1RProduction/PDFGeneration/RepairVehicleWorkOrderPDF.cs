...
{
    public class RepairVehicleWorkOrderPDF
    {
        Document document;
        ...
        private ObservableCollection<VehicleWorkDescription> vehicleWorkDescription;

        public RepairVehicleWorkOrderPDF(VehicleWorkOrder vwo,ObservableCollection<VehicleWorkDescription> vwd)
        {
        ...
        public Exception createWorkOrderPDF()
        {
            ...
            string filename = "VWOR" + vehicleWorkOrder.VehicleWorkOrderID + "_" + currentDate.ToString("_ddMMyyyy_HHmmss") + ".pdf";
            try
            {
        ...
        void CreatePage()
        {
            ...
            //paragraph = this.addressFrame.AddParagraph("Maintenance Frequency - " + (String.IsNullOrWhiteSpace(frequency) ? "Monthly" : frequency + unit));
            //paragraph.Format.Font.Name = "Calibri";
            //paragraph.Format.Font.Size = 12;
            ////paragraph.Format.SpaceBefore = "2cm";
            //paragraph.Format.Font.Bold = true;
            ...
            //mRow2.Cells[1].AddParagraph(vehicleWorkOrder.User.FullName);
            ...
            foreach (var items in vehicleWorkDescription)
            {
                foreach (var item in items.VehicleRepairDescription)
	            {
                    if (item.StrSequenceNumber != "Select")
                    {
                        ...
                        row2.Cells[0].AddParagraph(item.StrSequenceNumber);
                        ...
                        row2.Cells[1].AddParagraph(item.RepairDescription);
