...
{
    public class UpdateQuoteViewModel : ViewModelBase, IDataErrorInfo
    {
        ...
        public UpdateQuoteViewModel(string UserName, string State, List<UserPrivilages> Privilages, List<MetaData> md)
        {
        ...
        private void SearchQuote()
        {
            ...
            if (executeConn)
            {
                ...
                {
                    // Console.WriteLine("Quote number not available");
                }
            }
            else
            {
                ...
                //Console.WriteLine("Cannot retrieve database information, please try again later");
                Clear();
            }
        }
        ...
        string GetValidationError(string propertyName)
        {
            ...
            switch (propertyName)
            {
                case "SelectedQuoteNo": case "SelectedProjectName": case "SelectedSearchedCustomerName" :
        ...
        public ICommand BackCommand
        {
            get
            {
                return _backCommand ?? (_backCommand = new LogOutCommandHandler(() => Switcher.Switch(new QuotingMainMenu(_userName, _state, _privilages, metaData)), _canExecute));
            }
        }
        ...
        public ICommand NavHomeCommand
        {
            get
            {
                return navHomeCommand ?? (navHomeCommand = new LogOutCommandHandler(() => CheckQuoteForHomeName(), _canExecute));
            }
        }
        public ICommand NavQuotingCommand
        {
            get
            {
                return navQuotingCommand ?? (navQuotingCommand = new LogOutCommandHandler(() => CheckQuoteForQuoting(), _canExecute));
            }
        }
        ...
        public ICommand ClearFields
        {
            get
            {
                return _clearFields ?? (_clearFields = new LogOutCommandHandler(() => Clear(), _canExecute));
            }
        }
        ...
        private void CheckQuoteForHomeName()
        {
            if (!String.IsNullOrWhiteSpace(SelectedQuoteNo) || !String.IsNullOrWhiteSpace(SelectedProjectName))
            {
                ...
                {
                    Switcher.Switch(new MainMenu(_userName, _state, _privilages, metaData));
                }   
            }
            else
            {
        ...
        private void CheckQuoteForQuoting()
        {
            if (!String.IsNullOrWhiteSpace(SelectedQuoteNo) || !String.IsNullOrWhiteSpace(SelectedProjectName))
            {
                ...
                {
                    Switcher.Switch(new QuotingMainMenu(_userName, _state, _privilages, metaData));
                }
            }
            else
            {
